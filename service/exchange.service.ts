...
export const processExchangeService = async (data: ExchangeDTO) => {
  ...
  try {
    ...
    const [sourcePrice, targetPrice] = await Promise.all([
      ProductPrice.findOne({ where: { productId: data.sourceProductId, endAt: null }, transaction }),
      ProductPrice.findOne({ where: { productId: data.targetProductId, endAt: null }, transaction })
    ]);

    if (!sourceProduct || !targetProduct) throw new Error("Products not found");
    if (!sourcePrice || !targetPrice) throw new Error("Active pricing missing for products");

    const sourceStock = await Stock.findOne({ where: { productId: data.sourceProductId }, transaction });
    const targetStock = await Stock.findOne({ where: { productId: data.targetProductId }, transaction });

    if (!sourceStock || !targetStock) throw new Error("Stock records missing");

    const deductUnits = data.exchangeType === "box" 
      ? data.sourceQuantity * sourceProduct.unitsPerBox 
      : data.sourceQuantity;

    const sourceTotalUnits = calculateUnits.toTotalUnits(sourceStock.boxQuantity, sourceStock.singleQuantity, sourceProduct.unitsPerBox);
    
    if (sourceTotalUnits < deductUnits) {
      throw new Error("Not enough stock in source product");
    }

    const newSource = calculateUnits.toDisplayUnits(sourceTotalUnits - deductUnits, sourceProduct.unitsPerBox);
    const targetTotalUnits = calculateUnits.toTotalUnits(targetStock.boxQuantity, targetStock.singleQuantity, targetProduct.unitsPerBox);
    const newTarget = calculateUnits.toDisplayUnits(targetTotalUnits + deductUnits, targetProduct.unitsPerBox);

    // FIX: Using sellPricePerUnit from your model
    ...
    await sourceStock.update({ boxQuantity: newSource.boxes, singleQuantity: newSource.singles }, { transaction });
    await targetStock.update({ boxQuantity: newTarget.boxes, singleQuantity: newTarget.singles }, { transaction });
