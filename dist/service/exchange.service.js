...
exports.processExchangeService = void 0;
const models_1 = __importDefault(require("../models"));
const StockHistory_1 = require("../models/StockHistory");
const Stock_1 = require("../models/Stock");
const inventory_utils_1 = require("../utils/inventory.utils");
const { Stock, Product, ProductPrice, Exchange, StockHistory, sequelize } = models_1.default;
const processExchangeService = async (data) => {
    ...
    try {
        ...
        if (!sourcePrice)
            throw new Error(`No active price found for source product ${data.sourceProductId}`);
        if (!targetPrice)
            throw new Error(`No active price found for target product ${data.targetProductId}`);
        ...
        if (!sourceStock) {
            sourceStock = await Stock.create({
                ...
                boxQuantity: 0,
                singleQuantity: 0,
                containerType: Stock_1.ContainerType.BOX,
            }, { transaction });
        }
        if (!targetStock) {
        ...
        const deductUnits = data.exchangeType === "box"
            ...
            : data.sourceQuantity;
        const sourceTotalUnits = inventory_utils_1.calculateUnits.toTotalUnits(sourceStock.boxQuantity, sourceStock.singleQuantity, sourceProduct.unitsPerBox);
        if (sourceTotalUnits < deductUnits) {
            throw new Error("Not enough stock in source product");
        }
        const newSource = inventory_utils_1.calculateUnits.toDisplayUnits(sourceTotalUnits - deductUnits, sourceProduct.unitsPerBox);
        const targetTotalUnits = inventory_utils_1.calculateUnits.toTotalUnits(targetStock.boxQuantity, targetStock.singleQuantity, targetProduct.unitsPerBox);
        const newTarget = inventory_utils_1.calculateUnits.toDisplayUnits(targetTotalUnits + deductUnits, targetProduct.unitsPerBox);
        ...
        await StockHistory.create({
            ...
            actionType: StockHistory_1.HistoryActionType.EXCHANGE,
...
exports.processExchangeService = processExchangeService;
